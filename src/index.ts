...
const locales: { [key: string]: IMonetochkaConfig | IMonetochkaProxyConfig } = {
  af: { g: ' ', d: ',', p: 2, t: '!@' },
  af_NA: { h: 'af' },
  af_ZA: { h: 'af' },
  agq: { g: ' ', d: ',', p: 2, t: '@!' },
...
const placeSeparators = (
  value: number,
  precision: number,
  config: IMonetochkaConfig
): string => {
  const [integerPart, floatPart] = value.toFixed(precision).split('.');
